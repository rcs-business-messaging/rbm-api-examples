/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.rcsbusinessmessaging.v1.model;

/**
 * Opens the user's default calendar app and starts the new calendar event flow with the agent-
 * specified event data pre-filled.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the RCS Business Messaging API. For a detailed
 * explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class CreateCalendarEventAction extends com.google.api.client.json.GenericJson {

  /**
   * Event description. Maximum 500 characters.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String description;

  /**
   * Event end time.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private String endTime;

  /**
   * Event start time.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private String startTime;

  /**
   * Event title. Maximum 100 characters.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String title;

  /**
   * Event description. Maximum 500 characters.
   * @return value or {@code null} for none
   */
  public java.lang.String getDescription() {
    return description;
  }

  /**
   * Event description. Maximum 500 characters.
   * @param description description or {@code null} for none
   */
  public CreateCalendarEventAction setDescription(java.lang.String description) {
    this.description = description;
    return this;
  }

  /**
   * Event end time.
   * @return value or {@code null} for none
   */
  public String getEndTime() {
    return endTime;
  }

  /**
   * Event end time.
   * @param endTime endTime or {@code null} for none
   */
  public CreateCalendarEventAction setEndTime(String endTime) {
    this.endTime = endTime;
    return this;
  }

  /**
   * Event start time.
   * @return value or {@code null} for none
   */
  public String getStartTime() {
    return startTime;
  }

  /**
   * Event start time.
   * @param startTime startTime or {@code null} for none
   */
  public CreateCalendarEventAction setStartTime(String startTime) {
    this.startTime = startTime;
    return this;
  }

  /**
   * Event title. Maximum 100 characters.
   * @return value or {@code null} for none
   */
  public java.lang.String getTitle() {
    return title;
  }

  /**
   * Event title. Maximum 100 characters.
   * @param title title or {@code null} for none
   */
  public CreateCalendarEventAction setTitle(java.lang.String title) {
    this.title = title;
    return this;
  }

  @Override
  public CreateCalendarEventAction set(String fieldName, Object value) {
    return (CreateCalendarEventAction) super.set(fieldName, value);
  }

  @Override
  public CreateCalendarEventAction clone() {
    return (CreateCalendarEventAction) super.clone();
  }

}
